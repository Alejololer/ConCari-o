import { notFound } from "next/navigation";
import { getProduct, getProductTypes, getOccasions } from "@/lib/products";
import { CmsForm } from "@/components/organisms/CmsForm";

// id === "nuevo" → create form; otherwise edit an existing product.
export default async function CmsProductPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const isNew = id === "nuevo";
  const [product, productTypes, occasions] = await Promise.all([
    isNew ? undefined : getProduct(id),
    getProductTypes(),
    getOccasions(),
  ]);
  if (!isNew && !product) notFound();

  return (
    <div className="px-4 py-6 sm:px-6 sm:py-8">
      <h1 className="mb-7 font-display text-3xl sm:text-4xl font-bold text-rose">
        {isNew ? "Nuevo detalle" : `Editar · ${product!.name}`}
      </h1>
      <CmsForm product={product ?? undefined} productTypes={productTypes} occasions={occasions} />
    </div>
  );
}
